...
def  main():
  ...
  installer = "\installer\IPlugElementary Installer.exe"
   
  if demo:
    installer = "\installer\IPlugElementary Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugElementary manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugElementary-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugElementary-v" + FULLVERSIONSTR + "-win-demo.zip"
